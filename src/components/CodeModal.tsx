...
import { formatQrData, QR_DATA_TYPES, qrDataAsText, type SavedQrCode } from '../domain/qr'
      ...
      <div className="modal-box relative h-full w-full max-w-full space-y-4 rounded-none p-3 pt-12 sm:h-auto sm:max-h-[calc(100dvh-2rem)] sm:max-w-2xl sm:rounded-box sm:p-6">
        ...
        >
          <ShareButton
            ...
            showLabel
            title={form.name || code.name || text}
            {...(data.type === QR_DATA_TYPES.url ? { url: data.url } : { text })}
          />
        </FormButtons>
      </div>
    </div>
