...
suspend fun main() {
    ...
    startAppWithKoin {
        ...
        val mobileMenuStore = storeOf(false)
                            ...
                            div("lg:hidden fixed inset-0 z-[100] bg-base-100 text-base-content flex flex-col") {
                                ...
                                div("flex items-center justify-between px-4 py-3 border-b border-base-300") {
                                    div("flex items-center gap-3") {
                                        img("h-8 w-8 dark:invert") {
                                            attr("src", "/favicon.svg")
                                            attr("alt", "Code Hoover logo")
                                        }
                                        h2("text-lg font-semibold m-0") {
                                            translate(DefaultLangStrings.PageTitle)
                                        }
                                    }
                                    button("btn btn-ghost btn-circle") {
                                        ...
                                        iconXMark()
                                        clicks handledBy { mobileMenuStore.update(false) }
                                    }
                                }
                                ul("menu menu-lg flex-1 gap-2 p-4 bg-base-100 overflow-y-auto") {
                                    headerMenuItems(
                                        screenStore,
                                        fileInput,
                                        savedCodesStore,
...
private fun RenderContext.headerMenuItems(
    screenStore: Store<Screen>,
    fileInput: Tag<HTMLInputElement>,
